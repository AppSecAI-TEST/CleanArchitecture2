...
public class RepositoryContentProvider {
    ...
    public static synchronized void requestContacts(final RepositoryRequestGetContactsEvent event) {
        final List<PhoneContactItem> list = SerializableUtil.serializableToList(Admin.getInstance().getRepository().getFromCache(String.valueOf(event.getId()), event.getCacheType()));
        if (list != null) {
            EventBusController.getInstance().post(new RepositoryResponseGetContactsEvent()
                    .setResponse(list)
                    .setFrom(Repository.FROM_CACHE));
        } else {
            final RepositoryResponseGetContactsEvent responseEvent = (RepositoryResponseGetContactsEvent) Admin.getInstance().getContentProvider().getContacts();
            responseEvent.setFrom(Repository.FROM_CONTENT_PROVIDER);

            if (!responseEvent.hasError()) {
                Admin.getInstance().getRepository().putToCache(String.valueOf(event.getId()), event.getCacheType(), (Serializable) responseEvent.getResponse());
            }
